package com.example.trailblazer.domain;
...
import com.example.trailblazer.ui.StepDetector;
import com.example.trailblazer.data.DatabaseManager;
import com.example.trailblazer.data.GPSRepository;
import com.example.trailblazer.data.MovementUpdateListener;
...
import com.example.trailblazer.service.ElevationFinder;
import com.example.trailblazer.ui.MainActivity;
            ...
            public void onChanged(List<SavedLocation> locations) {
                ...
                if(savedLocations != null) {
                    for (SavedLocation savedLocation : savedLocations) {
                        ...
                        // Check if within 100 meters and not already entered
                        if (distanceToSavedLocation <= 100 ) {
                            savedLocationName = savedLocation.getName();
                            if(savedLocation.getLocationID() != currentLocationID){
                                updateNotification(); // Notify user
                                currentLocationID = savedLocation.getLocationID();
                            }
                        } else if (distanceToSavedLocation > 100) {
                            // If user is more than 100 meters away and has previously entered, set entered to false
